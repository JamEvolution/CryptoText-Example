...
def save_and_encrypt_notes():
    ...
    if len(title) == 0 or len(master_secret) == 0 or len(message) == 0:
        ...
        try:
            ...
            with open("my_secret.py", "w") as data_file:
                data_file.write(f"\n{title}\n{message_encrypted}\n")
        finally:
...
def decrypt_notes():
    message_encrypted = input_text.get(1.0, "end-1c")
    master_secret = master_secret_input.get()
